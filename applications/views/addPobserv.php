...
<section class="p-0">
    <div class="col-md-12 page-header">
        <h1 class="display-6">Project Checklist & Observations</h1>
    </div>
</section>
<div class="p-3">
    <div class="row">
...
<span class="input_error_message" id="occr_life_stage_error1"></span>
    ...
    <?php foreach($ppD as $prows) { if(!empty($prows->attribute_name)) { ?>           
                <div class="my-3">
        <?= ucfirst($prows->attribute_name);?></h6></label>  
        <?php if($prows->data_type=='boolean') { ?>
        <select name="<?= $prows->alias_name;?>" class="form-control"> 
           <option value="Yes">Yes</option>
           <option value="No">No</option> 
        </select>
        <?php } else {?>   
        <input type="<?= $prows->data_type;?>" name="<?= $prows->alias_name;?>" class="form-control"  <?php if($prows->is_mandatory=='Yes') { echo "required"; } ?> placeholder="<?= $prows->attribute_name;?>">
        <?php } ?> 
                </div>
                <?php } }?> 
        ...
        $.ajax({
                ...
                success: function(d) {
                    ...
                    if(d.success == 1) {
                        ...
                        window.location.href = '<?= site_url('PMS');?>';
                    }
                },
                error: function(e) {
