...
//getSpeciesDynamicCitation
$( document ).ready(function() {
	var sdd = $("#spDDetail").val();
    addEventListener("onclick", selectSpeciesValueMain(sdd));
    /*addEventListener("onclick", getSpeciesDynamicCitation(sdd));*/
    addEventListener("onclick", getSpeciesLocation(sdd));
});
...
const speciesLocationPlotter = (data) => {
    ...
    const iconStyle = new ol.style.Style({
        image: new ol.style.Icon({
            ...
            anchorYUnits: 'pixels',
            src: window.location.href.split('index.php')[0] + 'assets/img/dp-icons/icon.png'
        }),
    });
...
const selectSpeciesValueMain = (value) => {
	...
	$.ajax({
       ...
       $.ajax({
        ...
        processData: false,           
        success: function(d) {
        console.log(d.taxon.name.scientificName);
        console.log(d.taxon.name.specificEpithet);
        console.log(d.taxon.name.infraspecificEpithet);
        console.log(d.taxon.name.rank);
        console.log(d.taxon.name.authorship);
       ...
       console.log(d.taxon.name.code);
      console.log(d.taxon.name.nomStatus);
     //  alert(taxonID); return false;
...
/*function initDashboardTabNavigation() {

	$("#speciesCardDiv").on('click', function() {
		$("#speciesNavitemTabButton").click();
	});
	$("#datalayersCardDiv").on('click', function() {
		$("#datalayersNavitemTabButton").click();
	});
	$("#datasetsCardDiv").on('click', function() {
		$("#datasetsNavitemTabButton").click();
	});
	$("#reportsCardDiv").on('click', function() {
		$("#reportsNavitemTabButton").click();
	});
}*/
...
function DatasetViewMap(id, value)  {
 ...
 $('#DatasetVMap').modal('show');
  ...
  //alert(upID);
    ...
    //DatasetViewMapD.on("singleclick", function (evt) {
        /*var pos = ol.proj.transform(evt.coordinate, "EPSG:3857", "EPSG:4326");
        $("#checklist_location_latitude").val(pos[0]);
        $("#checklist_location_longitude").val(pos[1]);*/
        ...
        /*DatasetViewMapD.getLayers().forEach(function (layer) {
            if (layer.get("name") == "placemarker") {
                DatasetViewMapD.removeLayer(layer);
            }
        });*/
